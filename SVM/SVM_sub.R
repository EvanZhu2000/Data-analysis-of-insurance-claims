...
train <- sample.split(matrix(0, nrow=1 , ncol = nrow(data_sub)), SplitRatio = 0.75)
...
#Predictions
svm.radial.train <- predict(bestmod, data_sub[train,])
mean((svm.radial.train-data_sub$y[train])^2) # Training
svm.radial.test <- predict(bestmod, data_sub[!train,])
mean((svm.radial.test-data_sub$y[!train])^2) # Test
plot(bestmod,data_sub)
